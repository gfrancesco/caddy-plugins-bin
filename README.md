...
For each build, Caddy binaries are published under releases. 
Releases and images tags/versions follow the Caddy version numbers.
...
The images are [Alpine linux](https://www.alpinelinux.org/) only, based on the Caddy official ones. Images and binaries are built for the following platforms:
...
The final images are optimized for size. I use them to test network configurations on embedded devices and Mikrotik routers like the [RB5009](https://mikrotik.com/product/rb5009ug_s_in) family, which supports containers. Binaries can be used to add plugin funcionalities to existing installs, e.g. when Caddy is installed on [Home Assistant as an App](https://github.com/einschmidt/app-caddy-2).
...
This is how to build containers and artifacts after updates to the actions, dockerfile or to bump caddy's version:
